...
public final class NotificationManager {
  ...
  /// A task used to refresh pending state after synchronous removal APIs.
  @ObservationIgnored
  private var pendingRemovalRefreshTask: Task<Void, Never>?
  ...
  deinit {
    ...
    pendingRemovalRefreshTask?.cancel()
...
extension NotificationManager {
  ...
  /// Schedules a pending-state refresh after a synchronous removal call.
  internal func refreshPendingStateAfterRemoval() {
    pendingRemovalRefreshTask?.cancel()
    pendingRemovalRefreshTask = Task { [weak self] in
      guard let self else { return }
      await self.refreshPendingState()
    }
  }
...
extension NotificationManager {
  ...
  public func updateOptions(to options: UNAuthorizationOptions) async {
    ...
    do {
      ...
      logger.warning(
        "Failed to request additional notification options: \(error.localizedDescription)")
    }
  }
  ...
  public func scheduleRepeatingNotification(
    ...
    for day in days {
      ...
      await _schedule(
        ...
        body: body,
        category: category,
        type: .calendar(weekday: day.value, hour: hour, minute: minute, repeats: true),
        sound: sound,
        badge: badge,
        attachments: attachments,
        interruptionLevel: interruptionLevel,
...
extension NotificationManager {
  ...
  public func scheduleInactivityReminder(
    ...
    logger.info(
      "Scheduled inactivity reminder with interval=\(duration.timeInterval), id=\(inactivityReminderId)"
    )
  }
...
extension NotificationManager {
  ...
  ///
  /// Notification construction is performed on the main actor because several attachment
  /// factories render SwiftUI/UIKit/AppKit views. Attachment factories are evaluated in order.
  ///
  ...
  private func _schedule(
    ...
    let request = await NotificationRequestBuilder.makeRequest(
      id: id,
      title: title,
      subtitle: subtitle,
      body: body,
...
extension NotificationManager {
  ...
  public func removePendingNotification(id: String) {
    ...
    refreshPendingStateAfterRemoval()
